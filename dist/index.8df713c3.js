...
})({"1FKWU":[function(require,module,exports) {
...
class Text {
  ...
  draw() {
    ...
    .attr("dy", ".5em") // .attr("font-size", 11)
    .style("fill", palerRed).attr("opacity", .9).attr("class", function (d) {
...
class Topics {
  ...
  initTopic() {
    ...
    this.xAxis = d3.axisBottom(this.xScale).tickValues(this.topicData.map(d => d.chapter_start)).tickSize(-height).tickFormat("");
    this.yAxis = d3.axisLeft(this.yScale);
    const area = d3.area().x(d => this.xScale(d.data.chapter_midpoint)) //chapter_start))
    .y0(d => this.yScale(d[0])).y1(d => this.yScale(d[1])); // console.log("this.topicDat.data", this.topicData.data.chapter_title)

    const color = d3.scaleOrdinal().domain(this.stack.map(d => d.key)).range(this.colorArray).unknown("#ccc");
    this.svg = d3.select("#d3-container-TM").append("svg").attr("viewBox", [0, 0, width, height]).attr("class", "topic");
    this.svg.attr("class", "topic").append("g").attr("transform", `translate(0, ${height - marginBottom})`).call(this.xAxis).append("text").attr("class", "axis-label"); // const fillColorClass = ({ key }) => color(key).replace('#', '')
    ...
    const topics = this.svg.selectAll("path").on("mouseenter", function (d) {
      d3.selectAll('path').style("opacity", .4).attr("stroke", palerRed).attr('stroke-width', '.5');
      d3.select(this).raise().attr('stroke-width', '3').attr("stroke", ({
        key
      }) => color(key)) //"black")
      .style("opacity", 1).transition().duration(200);
    }).on("click", (event, d) => {
      // topicClickDesign();
...
// explainer
class Explainer {// HTML toggle button
  // annotations & suggestions
}

exports.default = Explainer;
},{"d3":"7FiJ2","../style.scss":"4wf1O"}],"4wf1O":[function() {},{}]},{},["1FKWU","4WM95","435K3"], "435K3", null)
